  ...
  IF S_PRCTR IS INITIAL.
    IF LT_BSID_SORT IS NOT INITIAL.
      ...
      SELECT BUKRS BELNR GJAHR BUZEI BUKRS_CLR BELNR_CLR GJAHR_CLR BUZEI_CLR CLRIN
        FROM BSE_CLR
        INTO CORRESPONDING FIELDS OF TABLE GT_BSE
        FOR ALL ENTRIES IN LT_BSID_SORT
...
FORM GET_DATA_FROM_TABL .
  ...
  SELECT BUKRS KUNNR KNAME PRCTR RACCT AWKEY BELNR BSCHL WAERS WRBTR DMBTR BUDAT BLDAT CR_PRD
         DUE_DATE AMT_REL NET_OS NOT_DUE OD_DAYS OD_DAYSDT AMT_OVD INVREV LR_NO LR_DATE BRAND
         KAM KAMNAME MANAGER PAY_TERMS UMSKS UMSKZ AUGDT AUGBL GJAHR BUZEI CPUDT XBLNR REBZG
         BLART SGTXT ZUONR VTWEG SPART VKORG KDMAT BSTKD IHREZ BSTDK ZLOCN LTEXT BZIRK MATNR
         HWSTE KBETR REGIO_TXT KVGR1 WERKS AGEING DUE_PERT_MTH DUE_NEXT_MTH DUE_AFT_NEXT_MTH
         SHP_TO_PARTY ORT01 NAME1 SORTL WRBTR1 NET_OS_DOC DIFFW
    FROM ZFI_SALESINV
